...
const Navbar = () => {
    ...
    const {user,setUser, setShowUserLogin,
        navigate,setSearchQuery,searchQuery} = useAppContext()
    ...
    return (
        <nav className="flex items-center justify-between px-4
         md:px-14 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative
          transition-all">
                ...
                {!user ? (
                    ...
                    <div className='relative group'>
                        ...
                        <ul className='hidden group-hover:block absolute
                         top-10 right-0 bg-white shadow
            ...
            <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                ...
                {!user ? (
                    ...
                    py-2 mt-2 bg-primary hover:bg-primary-dull transition text-white rounded-full text-sm">
                        Logout
                    </button>
                )}
